...
/***/ function(module, exports, __webpack_require__) {
    ...
    if (page == 'PRODUCT-GROUP')
    {
        (0, _index2.default)(document.querySelector('#group-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation:300 })
        .on('drop', function () {
            CIRCULATION.utils.showLoader();
            var positionRecord = {};
            var i = 0, num, $thisRec;
            $('#group-table tbody tr:visible').each(function (index, el) {
                $thisRec = $(this);
                num=index + parseInt(1);
                positionRecord[i] = {
                    "group_code": $thisRec.attr("data-grpid"),
                    "group_priority": num
                }
                i++;
            });
            var formData = new FormData();
            formData.append("pos_Record", JSON.stringify(positionRecord)); 
            $.ajax({
                type: 'POST',
                url: baseUrlPMD + 'Masters/UpdateGroupPriority',
                data: formData,
                dataType: 'json',
    ...
    if (page == 'UNIT-MASTER')
    {
        (0, _index2.default)(document.querySelector('#unit-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation: 300 })
            .on('drop', function () {
               
                    CIRCULATION.utils.showLoader();
                    var positionRecord = {};
                    var i = 0, no, $thisRec;
                    $('#unit-table tbody tr:visible').each(function (index, el) {
                        $thisRec = $(this);
                        no = index + parseInt(1);
                        positionRecord[i] = {
                            "unit_code": $thisRec.attr("data-untcode"),
                            "unit_priority": no
                        }
                        i++;
                    });
                    ...
                    $.ajax({
                        type: 'POST',
                        url: baseUrlPMD + 'AdminTools/UpdateUnitPriority',
                        data: formData,
                        dataType: 'json',
                        processData: false,  // tell jQuery not to process the data
                        contentType: false,   // tell jQuery not to set contentType
                        success: function (data) {
                            ...
                            if (data.status === 200) {
                                ...
                                $('#unit-table tbody tr:visible').each(function (index, el) {
                                    $(this).children('td').first().text(index + parseInt(1));
                                });
    ...
    if (page == 'PRODUCT-MASTER')
    {
        (0, _index2.default)(document.querySelector('#product-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation: 300 })
	.on('drop', function () {
	    CIRCULATION.utils.showLoader();
	    var positionRecord = {};
	    var i = 0, no, $thisRec;
	    $('#product-table tbody tr:visible').each(function (index, el) {
	        $thisRec = $(this);
	        no = index + parseInt(1);
	        positionRecord[i] = {
	            "product_code": $thisRec.attr("data-prdid"),
	            "product_priority": no
	        }
	        i++;
	    });
	    ...
	    $.ajax({
	        type: 'POST',
	        url: baseUrlPMD + 'Masters/UpdateProductsPriority',
	        data: formData,
	        dataType: 'json',
	        processData: false,  // tell jQuery not to process the data
	        contentType: false,   // tell jQuery not to set contentType
    ...
    if (page == 'BUREAU-MASTER') {
        (0, _index2.default)(document.querySelector('#bureau-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation: 300 })
            .on('drop', function () {

                CIRCULATION.utils.showLoader();
                var positionRecord = {};
                var i = 0, no, $thisRec;
                $('#bureau-table tbody tr:visible').each(function (index, el) {
                    $thisRec = $(this);
                    no = index + parseInt(1);
                    positionRecord[i] = {
                        "bureau_code": $thisRec.attr("data-burcode"),
                        "bureau_priority": no
                    }
                    i++;
                });
                ...
                $.ajax({
                    type: 'POST',
                    url: baseUrlPMD + 'Masters/UpdateBureauPriority',
                    data: formData,
                    dataType: 'json',
                    processData: false,  // tell jQuery not to process the data
                    contentType: false,   // tell jQuery not to set contentType
    ...
    if (page == 'EDITION-MASTER') {
        (0, _index2.default)(document.querySelector('#edition-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation: 300 })
            .on('drop', function () {

                CIRCULATION.utils.showLoader();
                var positionRecord = {};
                var i = 0, no, $thisRec;
                $('#edition-table tbody tr:visible').each(function (index, el) {
                    $thisRec = $(this);
                    no = index + parseInt(1);
                    positionRecord[i] = {
                        "edition_code": $thisRec.attr("data-edtcode"),
                        "edition_priority": no
                    }
                    i++;
                });
                ...
                $.ajax({
                    type: 'POST',
                    url: baseUrlPMD + 'Masters/UpdateEditionPriority',
                    data: formData,
                    dataType: 'json',
                    processData: false,  // tell jQuery not to process the data
                    contentType: false,   // tell jQuery not to set contentType
    ...
    if (page == 'ROUTE-MASTER') {
        (0, _index2.default)(document.querySelector('#route-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation: 300 })
            .on('drop', function () {

                CIRCULATION.utils.showLoader();
                var positionRecord = {};
                var i = 0, no, $thisRec;
                $('#route-table tbody tr:visible').each(function (index, el) {
                    $thisRec = $(this);
                    no = index + parseInt(1);
                    positionRecord[i] = {
                        "route_code": $thisRec.attr("data-rtcode"),
                        "route_priority": no
                    }
                    i++;
                });
                ...
                $.ajax({
                    type: 'POST',
                    url: baseUrlPMD + 'Masters/UpdateRoutePriority',
                    data: formData,
                    dataType: 'json',
                    processData: false,  // tell jQuery not to process the data
                    contentType: false,   // tell jQuery not to set contentType
    ...
    if (page == 'DROPPING-POINT-MASTER') {
        (0, _index2.default)(document.querySelector('#drop-table'), { mode: 'row', dragHandler: '.handle', onlyBody: true, animation: 300 })
            .on('drop', function () {

                CIRCULATION.utils.showLoader();
                var positionRecord = {};
                var i = 0, no, $thisRec;
                $('#drop-table tbody tr:visible').each(function (index, el) {
                    $thisRec = $(this);
                    no = index + parseInt(1);
                    positionRecord[i] = {
                        "drop_code": $thisRec.attr("data-drpid"),
                        "drop_priority": no
                    }
                    i++;
                });
                ...
                $.ajax({
                    type: 'POST',
                    url: baseUrlPMD + 'Masters/UpdateDroppingMasterPriority',
                    data: formData,
                    dataType: 'json',
                    processData: false,  // tell jQuery not to process the data
                    contentType: false,   // tell jQuery not to set contentType
