...
{
	$FileName="Other-Receipt-PDC-Summary-".date("F-j-Y").".xlsx";
...
<body class="hold-transition skin-blue sidebar-mini sidebar-collapse">
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                ...
                <div class="box">
                    <div class="box-body">
                        <form id="oim-pilot" method="post" target="_blank" action="<?php echo base_url('MISReports/OtherReceiptsPDCSummary?g_fe=xEdtsg'); ?>" onsubmit="return CIRCULATION.utils.formValidation(this);">
                          ...
                          foreach($status as $key => $value) {
                            ...
                            <div class="col-lg-2 col-md-3 col-sm-6 col-xs-12 hidden">
                                ...
                                <select id="select_date" name="select_date" class="form-control" tabindex="1">
                                    ...
                                    <?php $pay_type = Enum::getAllConstants('PayType');
                                        ...
                                        <option <?php if($key == PayType::Cheque){echo "selected";}?> value="<?php echo $key; ?>"><?php echo $value; ?></option>
                                        <?php } ?>
                                </select>
                            </div>
                            ...
                            <thead>
                                <tr>
                                    <td colspan="18" style="background-color: #e7f2f4; font-weight: bold; color:#000000;">
                                        <strong>Other Receipts PDC Report</strong> FROM <?php echo $_POST['from_date']; ?> to <?php echo $_POST['to_date'];?>&nbsp;<?php
                            ...
                            <tbody>
                              ...
                              if(count($scheme_rec)) {
                                  ...
                                  foreach($scheme_rec as $sh) {
                                      $grand_total += $sh->pdc_amount;
                                      if(!isset($payment[$sh->pdc_pay_type])){$payment[$sh->pdc_pay_type] = 0;}
                                      ...
                                      $payment[$sh->pdc_pay_type] += $sh->pdc_amount;
                                      $copy_type[$sh->copytype_name]["no"] += 1;
                                      $copy_type[$sh->copytype_name]["amt"] += $sh->pdc_amount;
                                      //$sch_cash += $sh->cash;
                                ...
                                <tr data-save="false" data-grpid="<?php //echo $ct->group_code; ?>">
                                    ...
                                    <td class="sh-number"><?php echo $sh->pdc_no; ?></td>
                                    <td class="sh-name"><?php echo $sh->pdc_date; ?></td>
                                    <td align="right" class="sh-add1"><?php echo $sh->pdc_sub_name; ?></td>
                                    <td align="right" class="sh-add1"><?php echo $sh->copytype_name; ?></td>
                                    <td align="right" class="sh-add1">
                                        <?php   if($sh->pdc_pay_type == PayType::Cheque){ echo "Cheque";} ?>
                                    </td>
                                    <td align="right" class="sh-add1"><?php echo moneyFormat($sh->pdc_amount); ?></td>
                                    <td align="right" class="sh-add1"><?php echo $sh->bank_name; ?></td>
                                    <td align="right" class="sh-add1"><?php echo $sh->pdc_chq_no; ?></td>
                                    <td align="right" class="sh-add1"><?php echo $sh->pdc_chq_date; ?></td>
                                    <td align="right" class="sh-add1"><?php echo $sh->pdc_remarks; ?></td>
                                    ...
                                    <td align="right" class="sh-add1"><?php echo $sh->pdc_scheme_code; ?></td>
                                    <td align="right" class="sh-add1">
                              ...
                              if(count($scheme_rec)) {
                                ...
                                <tr>
                                    ...
                                    <td align="right" bgcolor="#CCCCCC"><strong></strong><?php echo moneyFormat($payment[PayType::Cheque]);?></td>
                ...
                <?php 
                              ...
                              if($scheme_rec){
                                  ...
                                  foreach($scheme_rec as $sch){
                                      $sch_summ[$sch->group_name][$sch->pdc_pay_type] = $sch->amount;
                                      if(!isset($cgrp_total[$sch->group_name])) $cgrp_total[$sch->group_name]=0;
                                      if(!isset($pay_type_total[$sch->pdc_pay_type])) $pay_type_total[$sch->pdc_pay_type]=0;
                                      $pay_type_total[$sch->pdc_pay_type] += $sch->amount;
                ...
                <div class="box">
                    <div class="box-body table-responsive">
                        <table width="100%" class="table table-results">
                            <thead>
                                <tr>
                                    <td style="background-color: #e7f2f4; font-weight: bold; color:#000000;">
                                        <strong>Other Receipts PDC Summary</strong> FROM <?php echo $_POST['from_date']; ?> to <?php echo $_POST['to_date'];?>&nbsp;<?php
                                  $multi_scheme=isset($_POST['multi_sel_scheme'])?$_POST['multi_sel_scheme']:'';
                                  if($multi_scheme) {
                                      $scheme_record = json_decode(rawurldecode($multi_scheme),true);  echo "| Scheme: "; foreach($scheme_record as $sch){
                                          echo " ".$sch["Name"];
                                    ...
                                    <td style="background-color: #e7f2f4; font-weight: bold; color:#000000;"></td>
                                </tr>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td align="right"><strong><?php echo isset($sch_summ[$cpygrp["Name"]][PayType::Cheque])?$sch_summ[$cpygrp["Name"]][PayType::Cheque]:0; ?></strong></td>
                                </tr>
                                <?php
                                      }
                                  }
                                  else {
                                      echo "<tr><td colspan='2' class='no-records'><i class='fa fa-exclamation-circle' aria-hidden='true'></i> No Records Found!</td></tr>";
                                  }
                                  //if(count($scheme_rec)) {
                                ?>
                                <tr>
                                    <td bgcolor="#CCCCCC"><strong>TOTAL</strong></td>
                                    <td align="right" bgcolor="#CCCCCC"><strong><?php echo isset($pay_type_total[PayType::Cheque])?$pay_type_total[PayType::Cheque]:0; ?></strong></td>
                                </tr>
                                <?php //}?> 
                        </table>
                    </div>
    ...
    <!-- ./wrapper -->
    <script>
        var page = 'OTHER-RECEIPT-PDC-SUMMARY';
    </script>
